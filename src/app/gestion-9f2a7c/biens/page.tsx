...
export default async function AdminPropertiesPage({
  ...
  const pageHref = (p: number) =>
    `/gestion-9f2a7c/biens?${new URLSearchParams({ ...(q ? { q } : {}), page: String(p) })}`;

  return (
    <AdminShell userName={session.name}>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h1 className="text-2xl text-ink">Biens immobiliers</h1>
        <Link href="/gestion-9f2a7c/biens/nouveau" className="btn-gold">
          + Ajouter un bien
        </Link>
      </div>
      ...
      <form method="get" className="mt-4 flex gap-2">
        ...
        {q && (
          <Link href="/gestion-9f2a7c/biens" className="btn-ghost !py-2 text-sm">
            Réinitialiser
          </Link>
        )}
      </form>
        ...
        {properties.map((property) => (
          ...
          >
            ...
            <div className="flex items-center gap-4 sm:shrink-0">
              ...
              <Link
                href={`/gestion-9f2a7c/biens/${property.id}`}
